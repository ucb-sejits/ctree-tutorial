...
--------------------
The previous approach works really well for simple lambdas, like we used in the
example, but it has a problem. Our macro functions don't have the context
they're working on. Consider the following example:
...
Even though this is a perfectly valid code in Python. This is the code that
would be generated:
...
We will continue through an alternative implementation that, instead of
creating functions, inserts the equivalent code right where the function call
used to be. This way the scope will be the same but other problems arise. This
would mess with nested function calls and variables that are assigned with the
function return. Fortunately we can make our transformers handle those cases.
...
.. code:: python
    ...
    array_2 = np_map(lambda x: x-1, array_1)
...
Observe that now we're replacing the ``Call`` node by a ``MultiNode``. A
...
This works if we don't have nested calls. The following diagram shows what
should happen with nested function calls:
...
First we convert the call to ``b`` to a ``MultiNode``. Now this ``MultiNode``
...
.. code:: python
    ...
    array_sum = np_reduce(lambda x, y : x+y, array)
...
This transformer checks both ``Assign`` and ``Return`` nodes for the
``return_ref`` attribute and does the required modifications.

The code for this new approach can be found at
`<examples/np_functional_inline.py>`_ and this is the output for the example in
